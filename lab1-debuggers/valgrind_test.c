...
int main() {
  ...
  // This loop will allocate memory on the heap and then lose the pointer to that
  for (uninitialized_variable = 0; uninitialized_variable < 100; uninitialized_variable++) {
    ...
    *definitely_lost = (void*) malloc(7); // Give the pointer something else to
    ...
    free(*definitely_lost); // This frees the memory pointed to by definitely_lost
    free(definitely_lost); // This frees the memory pointed to by definitely_lost
  }
  ...
  // get back to the start of the block before freeing it
  possibly_lost -= 4;
  free(possibly_lost);
